...
\makeatother
\hypersetup{
  pdfkeywords={\xmpquote{hypertension; non-communicable diseases; vulnerability index; multiple correspondence analysis; multilevel logistic regression; Bangladesh; BDHS; external validation}},
  colorlinks=true,
  linkcolor={black},
  filecolor={Maroon},
  citecolor={blue},
...
\begin{longtable}[t]{lllll}
\caption{\label{tab:multitab}Results of multilevel logistic regression analysis of hypertension on NCD Vulnerability Index (NCDVI) adjusted for confounding and survey weights.}\\
\toprule
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.7\textwidth]{Figures/calibration_internal.png}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.7\textwidth]{Figures/calibration_external.png}
...
\subsection{Clinical and Policy Utility (Decision Curve Analysis)}\label{clinical-and-policy-utility-decision-curve-analysis}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.7\textwidth]{Figures/dca_internal.png}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.7\textwidth]{Figures/dca_external.png}
...
Moreover, similar factor scores correspond to the same levels of nominal variables, as determined by the interpretation of factor scores in multiple correspondence analysis. From this information, we deduce that the persons in tier 1 do not have a doctor-recommended exercise, diet, or smoking cessation plan and also have a healthy BMI, and that those in tier 5 exhibit the exact opposite characteristics. The following classifications are possible as per the increasing trend in risk:

\input{tier_classification.tex}
...
Based on this study's findings, an NCD Vulnerability Index constructed from health-behavior and household-environmental variables is a valuable tool for detecting and preventing hypertension at an early stage. Its internal and external validation establish it as a reliable indicator of hypertension risk. This study emphasizes the significance of promoting healthy habits and lifestyle choices for the prevention and management of hypertension.
...
\textbf{Ethics approval and consent to participate.} This study used publicly available, de-identified secondary data from the Bangladesh Demographic and Health Survey (BDHS) 2017-18 and BDHS 2022. The original surveys were reviewed and approved by the institutional review boards of ICF International and the Bangladesh Medical Research Council, and written informed consent was obtained from all respondents at the time of data collection. No new identifiable information was generated by the present analysis, and no additional ethics approval was required.

\textbf{Consent for publication.} Not applicable.

\textbf{Availability of data and materials.} The BDHS 2017-18 and BDHS 2022 datasets are available from The DHS Program (\url{https://dhsprogram.com}) upon registration and approval of a research request. The analysis code that reproduces all results, tables, and figures in this manuscript is openly archived on Zenodo (\url{https://doi.org/10.5281/zenodo.20719300}) and developed at \url{https://github.com/Tahsin2799/ncdvi-hypertension-bangladesh}.
...
\textbf{Authors' contributions.} A. F. M. Tahsin Shahriar and Nasrin Lipi contributed to the conceptual framework, methodology, data analysis, and manuscript writing. Both authors read and approved the final manuscript.

\textbf{Acknowledgements.} The authors thank The DHS Program and the Bangladesh Bureau of Statistics for making the BDHS data available.
...
\section*{Appendix A. Summary of Analytical Development}\label{appendix-a.-summary-of-analytical-development}
\addcontentsline{toc}{section}{Appendix A. Summary of Analytical Development}

The NCDVI was developed in a sequence of analytical stages. The stages were not pre-registered; each was triggered by a specific weakness identified in the preceding step, and each is reproducible from the R scripts in the archived repository. This appendix summarizes the path from the original six-input draft index to the final ten-input development index, the six-input sensitivity index, and the BDHS 2022 external validation.

\textbf{Stage 1. Index construction and expansion.} The original draft index used six clinical and anthropometric inputs (occupation, BMI, and the four clinician-advice items). A literature audit established that BDHS 2017-18 PR7 does not contain canonical behavioral risk variables (tobacco use, dietary intake, physical activity, alcohol) at the individual level, and that the realistic expansion path was through household-environmental variables. Five structural variables (cooking fuel, water source, toilet type, household crowding, and cooking location) were added, raising the count to eleven candidate inputs, later reduced to the final ten in Stage 6. The expansion preserved the monotonic dose-response while softening the Tier 1 to Tier 5 cliff in raw hypertension prevalence from about 72\% to about 62\%, showing that the new inputs absorbed part of the screening-driven concentration in the topmost tier without erasing the underlying gradient.

\textbf{Stage 2. Endogeneity sensitivity index.} Four of the inputs (the clinician-advice items) only fire after a respondent has been screened, so they encode prior healthcare contact. A parallel index built from only the non-endogenous structural inputs quantifies how much of the headline Tier 5 effect those items carry. The Tier 5 adjusted odds ratio fell from 15.79 in the full index to 3.22 in the structural-only index, an 80\% reduction, while the qualitative dose-response was preserved. The two indices measure complementary constructs: the ten-input version is a screening tool for already-contacted populations, and the six-input version targets populations not yet reached by primary care.

\textbf{Stage 3. Multilevel estimator selection.} An initial pseudo-maximum-likelihood fit (\texttt{WeMix::mix}) with raw DHS level weights returned an implausibly high intraclass correlation of 0.43, well above the 0.05 to 0.15 range in the BDHS hypertension literature. A structured investigation attributed the inflation to the pseudo-likelihood targeting the population-weighted log-likelihood with unscaled level-one sampling weights; a two-by-two decomposition (level-one weight scaling on or off, by level-two weight on or off) recovered a 35\% reduction from level-one scaling and a 69\% reduction from setting the level-two weight to one. The final primary specification is \texttt{lme4::glmer} with the sample weight as a frequency weight (Tier 5 OR 15.79, ICC 0.040), with the cluster-scaled \texttt{WeMix} fit (Tier 5 OR 27.10, ICC 0.329) retained as a supplementary survey-design-corrected estimate.

\textbf{Stage 4. Internal validation.} Validation followed the TRIPOD standard. Apparent AUC was 0.796 (95\% CI 0.787 to 0.805), tenfold cross-validated AUC was 0.793, and the Harrell bootstrap with two hundred resamples estimated optimism at 0.002, giving a bootstrap-corrected AUC of 0.794. The calibration intercept was near zero and the slope close to one across all three regimes. Paired DeLong tests confirmed that the ten-input NCDVI added 0.053 AUC over the confounders-only model, and the six-input sensitivity index added a smaller but still significant 0.008.

\textbf{Stage 5. Discrimination ceiling.} A build-up of realistic discrimination levers (continuous NCDVI score, continuous BMI, age-by-sex and sex-by-BMI interactions, and natural splines on age and BMI) raised the cross-validated AUC from 0.793 to 0.816. A gradient-boosted tree on the same feature set reached 0.811, placing the interpretable multilevel logistic regression at the discrimination ceiling achievable from PR7 variables. The higher AUCs of 0.90 and above reported in some machine-learning papers on BDHS hypertension come from variables outside PR7 or from methodological choices such as oversampling or subpopulation restriction that are not directly comparable.

\textbf{Stage 6. MCA robustness and input finalization.} Six sensitivity checks probed the MCA construction: the number of retained dimensions, the tier cutoff method (quintile, equal-width, Jenks, and k-means), the number of tiers (four to seven), substitution of \texttt{PCAmix} for the MCA fit, the Greenacre first-dimension inertia adjustment, and removal of each input in turn. Cooking location was effectively constant (97\% of respondents in a single category) and contributed no signal; removing it left the Tier 5 odds ratio unchanged and marginally improved the cross-validated AUC, so the production index was finalized at ten inputs and the sensitivity index at six. The Greenacre-adjusted inertia share for the first dimension was 91.2\%, supporting a single retained dimension.

\textbf{Stage 7. External validation on BDHS 2022.} The BDHS 2022 PR, HR, and IR files were audited for the inputs needed to transport the index. The four clinician-advice items were dropped from the 2022 questionnaire, so only the six-input sensitivity NCDVI is transportable; its six structural inputs all map to 2022 variables. Each woman aged eighteen and above in the 2022 biomarker subsample was projected onto the 2017-18 MCA dimension by the supplementary-points formula and assigned to a 2017-18 quintile tier, with about 84\% mapping to Tier 1, in line with documented gains in cooking fuel, water source, and sanitation between the two surveys. The raw hypertension prevalence rose monotonically from 15\% in Tier 1 to 38\% in Tier 5. A multilevel logistic regression on the 2022 frame (random intercept on PSU, the sex covariate dropped for the women-only sample, otherwise the development confounder set) gave a five-tier Tier 5 adjusted odds ratio of 4.03 (95\% CI 1.78 to 9.11) and a four-tier collapsed top-tier odds ratio of 3.73 (95\% CI 1.90 to 7.32); the four-tier collapse is the manuscript headline because Tier 4 alone held only 15 respondents. External cross-validated AUC was 0.713, and the external calibration slope and intercept were 0.901 and -0.150.

\section*{Appendix B. Supplementary Sensitivity Tables}\label{appendix-b.-supplementary-sensitivity-tables}
\addcontentsline{toc}{section}{Appendix B. Supplementary Sensitivity Tables}

\textbf{Table B.1. Multilevel estimator comparison on the ten-input primary NCDVI.} The primary specification (\texttt{glmer} with sample weight as frequency weight) is the literature-matched estimator. The supplementary specification (\texttt{WeMix::mix} pseudo maximum likelihood with cluster-scaled level-one weight) is the fully survey-design-corrected estimator.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{glmer}, weights as freq weights (primary) & 15.79 & {[}13.12, 19.00{]} & 0.137 & 0.040 \\
\texttt{WeMix::mix}, scaled level-one weight (supplementary) & 27.10 & {[}21.54, 34.08{]} & 1.609 & 0.329 \\
\end{longtable}
}

\textbf{Table B.2. MCA design alternatives.} Each row varies a single design choice. The production specification is the first row.
...
\textbf{Table B.3. Endogeneity sensitivity, Tier 5 odds ratio across NCDVI versions.}
...
\textbf{Table B.4. Discrimination ceiling, M0 to M6 build-up and gradient-boosted comparison.}
...
\section*{Appendix C. TRIPOD Reporting Checklist}\label{appendix-c.-tripod-reporting-checklist}
\addcontentsline{toc}{section}{Appendix C. TRIPOD Reporting Checklist}

The completed TRIPOD reporting checklist \citep{moons_tripod_2015} is provided in the table below and as a separate supplementary file.
...
21 & Supplementary information & Appendices A to E; supplementary files \\
22 & Funding & Declarations \\
\end{longtable}
}

\section*{Appendix D. Sample Flow}\label{appendix-d.-sample-flow}
\addcontentsline{toc}{section}{Appendix D. Sample Flow}

\textbf{BDHS 2017-18 development sample.} Starting from the PR7 file, restriction to adults aged 18 and above with valid blood pressure measurements yielded the analysis sample of N = 12,650 (9,084 non-hypertensive, 3,566 hypertensive). The full set of recoded confounders was non-missing on this sample. Random intercept on PSU covered 675 clusters.

\textbf{BDHS 2022 external-validation sample.} Starting from the BDHS 2022 Persons Recode (N = 132,463), the analytical flow restricted to women in the biomarker subsample, then to those aged 18 and above, then to those with non-missing values on the six NCDVI inputs and the outcome and confounders required by the multilevel model. After dropping two rows with missing education, the final analysis sample was N = 5,080 across 674 PSUs, of whom 835 were hypertensive. The hypertension outcome on BDHS 2022 used the mean of three valid SBP readings (variables \texttt{wbp9}, \texttt{wbp13}, \texttt{wbp22}), the mean of three valid DBP readings (variables \texttt{wbp10}, \texttt{wbp14}, \texttt{wbp23}), and the self-reported ``currently taking blood pressure medication'' indicator (\texttt{wbp19}), preserving structural parity with the 2017-18 outcome definition.

\section*{Appendix E. STROBE Reporting Checklist}\label{appendix-e.-strobe-reporting-checklist}
\addcontentsline{toc}{section}{Appendix E. STROBE Reporting Checklist}

The STROBE checklist for cross-sectional studies \citep{vonelm2007strobe} is provided in the table below and as a separate supplementary file.
...
\bottomrule\noalign{}
\endlastfoot
1 & Title and abstract: design and balanced summary & Title page; Abstract \\
2 & Background and rationale & Introduction \\
3 & Objectives & Introduction; Abstract \\
4 & Study design & Data and Variables (Source of Data); Methodology \\
5 & Setting & Data and Variables (Source of Data); Appendix D \\
6 & Participants: eligibility and selection & Data and Variables (Source of Data); Appendix D \\
7 & Variables: outcome, exposures, confounders & Data and Variables (Variables Used in Analysis) \\
8 & Data sources and measurement & Data and Variables; Methodology \\
9 & Bias & Methodology (endogeneity sensitivity); Discussion \\
10 & Study size & Data and Variables (N = 12,650); Appendix D \\
11 & Quantitative variables and groupings & Data and Variables (tiers, age bands); Methodology \\
12 & Statistical methods & Methodology (MCA, multilevel model, validation) \\
13 & Participants: numbers at each stage & Source of Data (participant flow); Appendix D \\
14 & Descriptive data & Results (Bivariate Analysis); Appendix D \\
15 & Outcome data & Results (Bivariate Analysis) \\
16 & Main results: adjusted estimates with confidence intervals & Results (Multilevel Logistic Regression) \\
17 & Other analyses: subgroups and sensitivity & Results (Sensitivity NCDVI; MCA Robustness; External Validation; Decision Curve Analysis) \\
18 & Key results & Discussion \\
19 & Limitations & Discussion \\
20 & Interpretation & Discussion \\
21 & Generalisability & Discussion (External Validation on BDHS 2022) \\
22 & Funding & Declarations \\
\end{longtable}
}
